...
for x in range(1, 4):
    images.append(
        pygame.transform.scale(
            pygame.image.load(os.path.join("jpg/SLIMES/SLIME5", "TD_slimeniebieski_" + str(x) + ".png")),
            (96, 96)))
class Slime_5_map_2(Enemy):
    def __init__(self):
        super().__init__()
    ...
    def draw(self, surface):
        ...
        modified_x = self.x - 35  # Modyfikacja wartości x
        modified_y = self.y - 50  # Modyfikacja wartości y
        surface.blit(self.img, (modified_x, modified_y))  # Modyfikacja wywołania blit()
